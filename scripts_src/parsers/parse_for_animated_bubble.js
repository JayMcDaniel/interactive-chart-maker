     ...
     */
    var parseForAnimatedBubble = function (input, chart_type, colors, chart) {
        ...
        output.timeline.dates = []; //e.g. years - to be populated by each row header.
        ...
        $("thead tr:eq(0) th:gt(0)", input).each(function (i) {
            var seriesObj = {
                name: $.trim($(this).text()),
                data: [output.timeline.data[0][i]],
                date: output.timeline.dates[0],
                type: chart_type,
                lineWidth: 0,
                marker: {
                    enabled: true
        ...
        output.timeline.animation = function () {
            ...
            animation_title.textContent = output.timeline.dates[0];
            ...
            chart_slider.setAttribute("max", output.timeline.dates.length - 1);
            ...
            chart_slider.oninput = function () {
                ...
                $.each(chart.series, function (i, e) {
                    e.update({
                        data: [output.timeline.data[new_time_index][i]]
                    });
                });

                animation_title.textContent = output.timeline.dates[new_time_index];
